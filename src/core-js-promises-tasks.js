...
function getPromise(number) {
  const prom = new Promise((fel, ret) => {
    if (number >= 0) {
      fel(number);
    }
    ret(number);
...
function getFirstResolvedPromiseResult(promises) {
  const prom = Promise.any(promises);
  return prom;
}
...
function getFirstPromiseResult(promises) {
  const prom = Promise.race(promises);
  return prom;
}
...
function getAllResult(promises) {
  const prom = Promise.all(
    promises.map((promis) => promis.then((res) => res).catch(() => null))
  );
  return prom;
}
...
function queuePromises(promises) {
  const prom = promises.reduce(
    (res, pro) => res.then((p) => pro.then((val) => `${p}${val}`)),
    Promise.resolve('')
  );
  return prom;
}
